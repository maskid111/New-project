...
export default function XProfileSection({ address, profile, onConnect, loading }) {
  ...
  if (profile) {
    return (
      <div className="w-full rounded-xl border border-emerald-700/40 bg-surface-elevated/90 p-5">
        <p className="text-xs uppercase tracking-wide text-emerald-300">X Connection</p>
  ...
  return (
    <div className="w-full rounded-xl border border-slate-700 bg-surface-elevated/90 p-6 shadow-xl">
      ...
      <h2 className="mt-1 text-xl font-semibold tracking-tight">Connect your X account</h2>
      <p className="mt-2 text-sm text-slate-300">
        Complete real OAuth to link your X profile and personalize your ParrotPass card.
      </p>

      <p className="mt-3 break-all text-xs text-slate-400">Wallet: {address}</p>
